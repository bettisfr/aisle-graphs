function [budget_out, cost_out, reward_out] = greedy_partial_row(input, budget)
    ...
    for i=1:size_row
        for j=1:size_column
            ...
            cumulative_cost_1(i, j) = (j - 1);
            cumulative_cost_2(i, size_column - j + 1) = (j - 1);
        end
    end
    ...
    while sum(sum(cumulative_reward_1)) > 0 || sum(sum(cumulative_reward_2)) > 0
        if current_side == 1
            distance_side = abs(current_row - (1:size_row)');
            loop_heuristic = cumulative_reward_1(:, size_column)./(cumulative_cost_1(:, size_column) + distance_side);
            muted_loop_heuristic = cumulative_reward_1./(2 * cumulative_cost_1 + repmat(distance_side, 1, size_column));
            ...
            loop_cost = abs(current_row - best_loop_index) + 2*cumulative_cost_1(best_loop_index, size_column) + abs(best_loop_index - ending_row);
            muted_loop_cost = abs(current_row - best_muted_row) + 2*cumulative_cost_1(best_muted_row, best_muted_col) + abs(best_muted_row - ending_row);
            if (best_loop_heuristic >= best_muted_heuristic) && (loop_cost <= (budget - total_cost))
                total_cost = total_cost + abs(current_row - best_loop_index) + cumulative_cost_1(best_loop_index, size_column);
            ...
            elseif (muted_loop_cost <= (budget - total_cost))
                total_cost = total_cost + abs(current_row - best_muted_row) + 2*cumulative_cost_1(best_muted_row, best_muted_col);
...
%                 total_reward = total_reward + accumulated_reward;
                ...
                tmp_a = abs(current_row - (1:size_row))';
                tmp_b = abs((1:size_row) - ending_row)';
                tmp_sum = tmp_a + tmp_b;
                temp_cost = repmat(tmp_sum, 1, size_column) + 2*cumulative_cost_1;
...
%                 end
        ...
        elseif current_side == 2
            ...
            muted_loop_heuristic = cumulative_reward_2./(2 * cumulative_cost_2 + repmat(distance_side, 1, size_column));
            [best_loop_heuristic, best_loop_index] = max(loop_heuristic);
            best_muted_heuristic = max(muted_loop_heuristic(:));
            [best_muted_row, best_muted_col] = find(muted_loop_heuristic == best_muted_heuristic, 1);
            loop_cost = abs(current_row - best_loop_index) + cumulative_cost_2(best_loop_index, 1) + abs(best_loop_index - ending_row);
            muted_loop_cost = abs(current_row - best_muted_row) + 2*cumulative_cost_2(best_muted_row, best_muted_col) + abs(best_muted_row - ending_row) + cumulative_cost_2(ending_row, 1);
            if (best_loop_heuristic >= best_muted_heuristic) && (loop_cost <= (budget - total_cost))
                total_cost = total_cost + abs(current_row - best_loop_index) + cumulative_cost_2(best_loop_index, 1);
                ...
                cumulative_reward_1(best_loop_index, :) = 0;
            elseif (muted_loop_cost <= (budget - total_cost))
                total_cost = total_cost + abs(current_row - best_muted_row) + 2*cumulative_cost_2(best_muted_row, best_muted_col);
...
%                 total_reward = total_reward + accumulated_reward;
                ...
                temp_cost = abs(current_row - (1:size_row))' + cumulative_cost_2(:, 1) + abs((1:size_row) - ending_row)';
                reachable = (temp_cost <= (budget - total_cost));
                cumulative_reward_1(~reachable, :) = 0;
...
%                 end
    ...
    if current_side == 1
        total_cost = total_cost + abs(current_row - ending_row);
    ...
    if current_side == 2 % Will only be at side 2 if the row needed to reach the ending has already been traversed
        total_cost = total_cost + abs(current_row - ending_row) + cumulative_cost_2(current_row, 1);
...
%         end
    ...
    % for i=1:size(tour)
    %     num = tour(i);
    %     from = ceil(num / size_row) -1;
    %     to = mod(num-1, size_row);
    %     fprintf('[%d %d]\n', from, to);
    % end
end
