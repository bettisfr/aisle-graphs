from algorithms import *
import matplotlib.pyplot as plt
from util import *


input = 1010

rewards = read_input_from_file_id(input)
[m, n] = rewards.shape

# min_budget = 2 * n
# step_budget = n
max_budget = n * m + 3 * m
max_rewards = np.sum(read_input_from_file_id(input))

# budgets = range(min_budget, max_budget+step_budget, step_budget)
budgets = np.linspace(0, max_budget, num=20)
size = len(budgets)

colors = ['dodgerblue', 'blue', 'black', 'lightcoral', 'red']
styles = ['dashed', 'solid', 'solid', 'dashed', 'solid']
algs = ['gfr', 'ofr', 'oprsc', 'gpr', 'hprgc']
algs_long = ['GFr', 'OFr', 'OSc', 'GPr', 'HGc']
alg_plot = [True, True, True, True, True]

rewards = np.zeros((len(algs), len(budgets)))
costs = np.zeros((len(algs), len(budgets)))

for k in range(0, len(algs)):
    if not alg_plot[k]:
        continue

    alg = algs[k]

    file_path = 'output/%s/out_%s-%d.txt' % (alg, alg, input)
    with open(file_path) as fp:
        for cnt, line in enumerate(fp):
            if cnt < len(budgets):
                data = line.strip().split(",")
                costs[k][cnt] = float(data[1])
                rewards[k][cnt] = float(data[2])

rewards = 100 * rewards / max_rewards
budgets = 100 * np.array(budgets) / budgets[-1]

plt.clf()
fig = plt.figure(figsize=(2.5, 2.5))
plt.rc('text', usetex=True)
plt.rc('font', family='serif')
plt.xlabel('$B (\\%)$')
plt.ylabel('$R (\\%)$')
plt.title('$G = [%d \\times %s]$' % (m, n))

axes = plt.gca()
axes.set_xlim([0, 100])
axes.set_ylim([0, 100])

for k in range(0, len(algs)):
    if not alg_plot[k]:
        continue

    plt.plot(budgets, rewards[k], c=colors[k], label=algs_long[k], linewidth=0.75, linestyle=styles[k])

plt.legend(loc='lower right', prop={'size': 6})

plt.tight_layout(pad=0.5)
plt.savefig('plot_algs_single_%d.pdf' % input)
