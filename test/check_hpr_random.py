...
def parse_args():
    p = argparse.ArgumentParser(description="C++ HPR random-instance feasibility checker.")
    ...
    p.add_argument("--seed-base", type=int, default=5000, help="Base seed (seed_base + instance_idx)")
    return p.parse_args()
...
def main() -> int:
    ...
    print(
        f"C++ HPR feasibility check on {num_instances} random instances "
    ...
    for inst_idx in range(num_instances):
        ...
        for budget in budgets:
            hpr = run_cpp_alg("hpr", budget, num_rows, num_internal_cols, seed)
            total_points += 1

            bad = []
            if hpr["cost"] > budget:
                bad.append(f"hpr cost > budget ({hpr['cost']} > {budget})")

            hpr_path_cost = compute_cycle_cost(hpr["cycle"])
            if hpr_path_cost != hpr["cost"]:
                bad.append(f"hpr cost != path cost ({hpr['cost']} vs {hpr_path_cost})")
