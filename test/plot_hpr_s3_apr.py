...
def run_cpp_alg(alg_name: str, budget: int, rows: int = None, cols: int = None, seed: int = None,
    ...
    cmd = [str(CPP_BIN), "-algorithm", alg_name, "-budget", str(budget), "-log", "0"]
    ...
    out = {"reward": None, "cost": None, "cycle": [], "grid": []}
...
def draw_instance_with_path(ax, grid, cycle, title, reward_value, cost_value, status_text=""):
    ...
    ax.imshow(full, cmap="Pastel1", interpolation="nearest",
              vmin=np.min(full), vmax=np.max(full) if np.max(full) > 0 else 1, alpha=0.55)
    ...
    lines = [title, f"reward={reward_value:.2f}, cost={cost_value:.2f}"]
    if status_text:
        lines.append(status_text)
    ax.set_title("\n".join(lines), fontsize=10, color=("#1b7f3b" if status_text.startswith("OK") else "black"))
...
def parse_args():
    p = argparse.ArgumentParser(description="Plot APR vs HPR-S3 on one aisle-grid instance.")
    ...
    p.add_argument("--out", type=str, default="test/output/plot_hpr_s3_apr.pdf", help="Output figure path")
    return p.parse_args()
...
def main() -> int:
    ...
    if args.input_csv:
        ...
        apr_cpp = run_cpp_alg("apr", args.budget, input_csv_path=input_csv)
        s2_cpp = run_cpp_alg("hpr-s3", args.budget, input_csv_path=input_csv)
    else:
        apr_cpp = run_cpp_alg("apr", args.budget, rows=args.rows, cols=args.cols, seed=args.seed)
        s2_cpp = run_cpp_alg("hpr-s3", args.budget, rows=args.rows, cols=args.cols, seed=args.seed)

    grid = apr_cpp.get("grid", [])
    if not grid:
        raise RuntimeError("Cannot parse grid from C++ output")
    if s2_cpp.get("grid") and s2_cpp["grid"] != grid:
        raise RuntimeError("C++ returned different random grids across APR/HPR-S3 calls")
    ...
    if apr_cpp["cost"] > args.budget + 1e-9:
        raise RuntimeError(f"APR infeasible: cost={apr_cpp['cost']} > budget={args.budget}")
    if s2_cpp["cost"] > args.budget + 1e-9:
        raise RuntimeError(f"HPR-S3 infeasible: cost={s2_cpp['cost']} > budget={args.budget}")

    fig, axes = plt.subplots(1, 2, figsize=(max(10, n * 1.6), max(4.8, m * 0.8)), constrained_layout=True)
    fig.suptitle(
        f"Single-instance APR vs HPR-S3 (m={m}, n={n} internal cols, graph cols={n + 2}, B={args.budget})",
        fontsize=12,
    )

    draw_instance_with_path(axes[0], grid, apr_cpp["cycle"], "APR", float(apr_cpp["reward"]), float(apr_cpp["cost"]), "OK (cost<=B)")
    draw_instance_with_path(axes[1], grid, s2_cpp["cycle"], "HPR-S3", float(s2_cpp["reward"]), float(s2_cpp["cost"]), "OK (cost<=B)")
